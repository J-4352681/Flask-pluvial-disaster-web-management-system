...
class Config_forms(FlaskForm):
    ...
    sort_users = SelectField("Criterio de ordenamiento de los usuarios", choices=['username', 'first_name', 'last_name', 'email']) #, validators=[DataRequired()]) 
    sort_meeting_points = SelectField("Criterio de ordenamiento de los puntos de encuentro", choices=['name', 'direction', 'coordinates', 'telephone', 'email']) #, validators=[DataRequired()]) 
