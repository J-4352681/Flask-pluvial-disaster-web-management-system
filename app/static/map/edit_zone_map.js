function create_polygon(map, input, areaColor) {
  var polygon;
  if (input.val() != '') {
    let coord = JSON.parse(input.val().replaceAll("'", '"'));
    polygon = L.polygon(coord, {color: areaColor});
    if(coord.length == 1) polygon.setStyle({weight: 12});
  }
...
$(document).ready(function() {
  ...
  polygon.on('mouseup', function() {
    if(JSON.parse(json_input.val().replaceAll("'", '"')).length > 1) polygon.setStyle({weight: 3});
  });
